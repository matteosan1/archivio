...
curl -s -X POST -H 'Content-type:application/json' --data-binary '{"add-field":{"name":"unanimita","type":"text_general","multiValued":false,"docValues":false,"indexed":true,"stored":true}}' http://localhost:$PORT/solr/$CORE/schema
...
curl -s -X POST -H 'Content-type:application/json' --data-binary '{"add-field":{"name":"num_contestuale","type":"pint","multiValued":false,"docValues":false,"indexed":true,"stored":true}}' http://localhost:$PORT/solr/$CORE/schema
...
curl -s -X POST -H 'Content-type:application/json' --data-binary '{"update-field":{"name":"anno","type":"pint","docValues":true,"multiValued":false,"indexed":true,"stored":true}}' http://localhost:$PORT/solr/prova5/schema
curl -s -X POST -H 'Content-type:application/json' --data-binary '{"add-field":{"name":"private","type":"pint","docValues":true,"multiValued":false,"indexed":true,"stored":true}}' http://localhost:$PORT/solr/$CORE/schema
